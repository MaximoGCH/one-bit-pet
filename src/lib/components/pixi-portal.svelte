...
<script lang="ts">
	import { getPixiContainer } from '$lib/state/pixi-app';

	import * as PIXI from 'pixi.js';
	import { afterUpdate, createEventDispatcher, getContext, onMount, setContext } from 'svelte';
	import { createApplyProps, getRenderer } from 'svelte-pixi';
	 ...
	 * @type {string}
	 */
	export let accessiblePointerEvents: $$Props['accessiblePointerEvents'] = 'auto';
	/**
	 ...
	 */
	export let cacheAsBitmapMultisample: $$Props['cacheAsBitmapMultisample'] = PIXI.MSAA_QUALITY.NONE;
	/**
	 ...
	 * @type {number}
	 */
	export let cacheAsBitmapResolution: $$Props['cacheAsBitmapResolution'] = undefined;
	/**
	...
	const { applyProp, applyProps } = createApplyProps<PIXI.Container, $$Props>(instance);
	...
	onMount(() => {
		...
		const parent = getPixiContainer()?.addChild(_instance);
		...
		return () => {
			_instance?.destroy();
			getPixiContainer()?.removeChild(_instance);
			invalidate();
		};
	});
	afterUpdate(() => {
